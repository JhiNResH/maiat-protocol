...
var onCronTrigger = (runtime2) => {
  ...
  const reviewData = runtime2.runInNodeMode(fetchMaiatReviews, consensusMedianAggregation())().result();
  runtime2.log(`Fetched data for ${reviewData} tokens from Maiat API`);
  const mockTokens = [
    {
  ...
  for (const token of mockTokens) {
    runtime2.log(`Token ${token.tokenAddress.slice(0, 10)}...: ` + `rating=${token.avgRating}/5, reviews=${token.reviewCount}, ` + `trustScore=${token.trustScore}/100`);
  }
  runtime2.log(`✅ Computed trust scores for ${mockTokens.length} tokens`);
  runtime2.log("Next step: Write batch update to TrustScoreOracle on Base Sepolia");
  return {
