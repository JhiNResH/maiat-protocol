...
async function fetchTrustScore(address, chain = "base") {
    const headers = {
...
function formatTrustReport(data) {
...
server.tool("maiat_check_trust", "Check the trust score of any on-chain address. Returns a 0-10 score with risk level, flags, and detailed breakdown. Use this before interacting with any unknown address.", {
...
server.tool("maiat_check_token", "Check if a token contract is safe to interact with. Returns safety assessment including honeypot detection, rug pull risk, and liquidity analysis.", {
...
server.tool("maiat_batch_check", "Check trust scores for multiple addresses at once. Returns a summary table. Useful for evaluating a set of addresses before batch operations.", {
    addresses: z.array(z.string()).describe("Array of addresses to check"),
    chain: z.string().optional().describe("Chain (base, ethereum, bnb). Default: base"),
}, async ({ addresses, chain }) => {
    const results = ["| Address | Score | Risk | Type | Flags |", "|---------|-------|------|------|-------|"];
    for (const addr of addresses.slice(0, 10)) {
        try {
            const data = await fetchTrustScore(addr, chain || "base");
            const emoji = formatRiskEmoji(data.risk);
            const shortAddr = `${addr.slice(0, 6)}...${addr.slice(-4)}`;
            results.push(`| \`${shortAddr}\` | ${emoji} ${data.score}/10 | ${data.risk} | ${data.type} | ${data.flags.slice(0, 3).join(", ")} |`);
        }
        catch {
...
server.tool("maiat_submit_review", "Submit a trust review for a contract address. Costs 2 Scarab points. Quality reviews earn 3-10 Scarab rewards.", {
...
server.tool("maiat_get_interactions", "Discover which contracts a wallet has interacted with on Base. Shows reviewable contracts.", {
    wallet: z.string().describe("Wallet address to check (0x...)"),
}, async ({ wallet }) => {
    try {
        const headers = { "User-Agent": "maiat-mcp-server/0.2.0" };
        if (API_KEY)
            headers["Authorization"] = `Bearer ${API_KEY}`;
...
server.tool("maiat_get_passport", "Get a wallet's reputation passport — trust level, Scarab balance, review history, and fee tier.", {
...
server.tool("maiat_defi_info", "Get trust data for a DeFi protocol by name or address. Examples: 'usdc', 'aerodrome', 'aave', '0x833589...'", {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(res.status === 404 ? `DeFi protocol "${query}" not found` : `HTTP ${res.status}`);
        const data = await res.json();
        const e = data.entity;
        const text = [
            `## ${e.name}\n`,
...
// Tool: Agent info
server.tool("maiat_agent_info", "Get trust data for an AI agent by name or address. Examples: 'aixbt', 'virtuals', 'luna', '0x4f9fd6...'", {
    ...
    try {
        ...
        if (!res.ok)
            throw new Error(res.status === 404 ? `Agent "${query}" not found` : `HTTP ${res.status}`);
        const data = await res.json();
        const e = data.entity;
        const text = [
            `## ${e.name}\n`,
...
server.resource("maiat-docs", "maiat://docs", async () => ({
    contents: [
        {
            uri: "maiat://docs",
