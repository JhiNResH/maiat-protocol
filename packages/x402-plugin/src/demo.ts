...
import { createTrustGatedClient, X402TrustError, X402PriceError } from "./index.js";
...
async function main() {
  ...
  try {
    const result = await client.fetch(
      "https://maiat-protocol.vercel.app/api/v1/trust-gate?agent=0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913"
    );
    console.log(`  Status: ${result.response.status}, Paid: ${result.paid}, Trust-checked: ${result.trustChecked}`);
    ...
    if (e instanceof X402TrustError) {
      console.log(`  Trust blocked: ${e.message}\n`);
    } else if (e instanceof X402PriceError) {
