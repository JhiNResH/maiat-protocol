...
export function maiatFunctionConfigs(config: MaiatVirtualsConfig = {}) {
  ...
  const checkTokenConfig: GameFunctionConfig<{ address: string; chain?: string }> = {
  ...
  const trustSwapConfig: GameFunctionConfig<{
    swapper: string;
    tokenIn: string;
    tokenOut: string;
    amount: string;
  ...
  const reportOutcomeConfig: GameFunctionConfig<{
    jobId: string;
    outcome: string;
    reporter?: string;
    note?: string;
...
export async function createMaiatWorker(config: MaiatVirtualsConfig = {}) {
  ...
  const functions = [
    ...
    new GameFunction(configs.checkTokenConfig as GameFunctionConfig<Record<string, string>>),
    new GameFunction(configs.trustSwapConfig as GameFunctionConfig<Record<string, string>>),
    new GameFunction(configs.reportOutcomeConfig as GameFunctionConfig<Record<string, string>>),
  ];
  ...
  return new GameWorker({
    ...
    description:
      "Checks on-chain trust scores, token safety, swap verification, and outcome reporting using the Maiat Protocol. Use before any financial transaction.",
    functions,
  });
}
