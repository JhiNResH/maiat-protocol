...
var onCronTrigger = (runtime2) => {
  runtime2.log("\uD83D\uDD31 Maiat Trust Score Workflow triggered");
  ...
  const reviewDataRaw = runtime2.runInNodeMode(fetchMaiatReviews, consensusMedianAggregation())().result();
  runtime2.log(`Fetched review metric: ${reviewDataRaw}`);
  const aiSentiment = runtime2.runInNodeMode(fetchGeminiSentiment, consensusMedianAggregation())().result();
  runtime2.log(`Gemini AI sentiment score: ${aiSentiment}`);
  const tokens = [
  ...
  for (const t of tokens) {
    runtime2.log(`Token ${t.tokenAddress.slice(0, 10)}...: score=${t.score}/100, reviews=${t.reviewCount}`);
  }
  const txHash = writeTrustScores(runtime2, network248.chainSelector.selector, evmConfig, tokens);
  runtime2.log(`✅ Trust scores written onchain. tx: ${txHash}`);
  return {
...
function computeTrustScore(avgRating, reviewCount, onchainBase, aiSentiment) {
  const reviewScore = Math.min(100, avgRating * 20);
  const communityScore = Math.min(100, reviewCount * 5);
  const weighted = Math.round(onchainBase * 0.4 + reviewScore * 0.3 + communityScore * 0.2 + aiSentiment * 0.1);
  return Math.max(0, Math.min(100, weighted));
}
function scoredToken(addr, avgRating, reviewCount, aiSentiment) {
  return {
    tokenAddress: addr,
...
var fetchMaiatReviews = (nodeRuntime) => {
...
var fetchGeminiSentiment = (nodeRuntime) => {
  const httpClient = new ClientCapability2;
...
function writeTrustScores(runtime2, chainSelector, evmConfig, tokens) {
  runtime2.log(`Writing ${tokens.length} trust scores to consumer: ${evmConfig.consumerAddress}`);
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("address[] tokens, uint256[] scores, uint256[] reviewCounts, uint256[] avgRatings"), [addresses, scores, reviewCounts, avgRatings]);
  ...
  runtime2.log(`Transaction: https://sepolia.basescan.org/tx/${txHash}`);
  return txHash;
}
async function main() {
